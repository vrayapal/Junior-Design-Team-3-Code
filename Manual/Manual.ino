...
void homeLA (){
  const float homingSpeedInMMPerSec = 25.0;
  const float maxHomingDistanceInMM = 720;   // since my lead-screw is 38cm long, should never move more than that
  ...
  stepperLA.setStepsPerMillimeter(40 * 1);
...
void home1 (){
  const float homingSpeedInStepsPerSec = 50.0;
  const float maxHomingDistanceInMM = 720;   // since my lead-screw is 38cm long, should never move more than that
...
void loop() {
  ...
  */
  ...
  if (yPosition2 >=100){
      stepper2.setSpeedInStepsPerSecond(80);
